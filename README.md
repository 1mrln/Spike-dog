...
## What it is 
We build a robotic dog with a focus on social interaction. It can move freely around the room and avoids obstacles. Additionally, it shows different reactions to various colors. If it detects the color yellow, it "greets" by wagging its head and tail, and barks happily. On magenta, it stops, barks and sounds an alarm. At the touch of a button, it starts the "heel" function, in which it moves at a defined distance from a person/object. 
In the following you can see a video with the functions. The code will be explained later on. 
...
At the beginning we wanted to develop a robotic dog that improves your security at home by patrolling your house and sounding an alarm. In addition, it was supposed to stand up and bark to scare off strangers. 
The first prototype we've built, consisted of two wheels, two motors, and a large plate. It was able to move around and bark at the colour red. You can see the prototype in the following picture. 
![prototype](prototype.jpg)

We chose this idea, because we wanted to provide safety without putting a living being in danger. But while building we decided that we wanted to focus more on the social aspect of human-machine-interaction. That's why we switched our project goal: Our robotic dog should be able to interact with you and foster the relationship with you by looking cute and approachable.

In the following we will guide you through the process of building and programming your own robotic dog. We will also explain why we chose this sepcific design and according functions. 
...
Our design was based on a [YouTube video](https://www.youtube.com/watch?v=efhQDqzQOD4) that we found. After comparing different examples we chose this design, because it fitted our requirements the best: 
1. The design is quite realistic: It has dog-like features like ears or greeting you with a wagging tail. Because of the familiarity with the most typical features of a real dog, we expeect the interaction threshold to be lower compared to other designs. 
2. The positioning of the distance sensor and its look tries to imitate the eyes of a puppy. This works as an easy non-verbal communication tool.
3. The size of the robot is intentionally small, compact and robust. This makes it convenient to carry the robot around and play with it.
...
To program our Spike dog we used the *Lego Spike App* and its programming user interface. After opening the App you will see a variety of tutorials and example projects. We recommend the *"First steps"-Tutorial* because it will shortly introduce you to crucial components and commands in a playful manner. 
...
Now your Spike Dog will move forward in a straight light, after you start the program. 

**Please be careful**: Depending on the position of the motors you might need to change the direction of the wheels spin. You can do this by clicking on the arrows in the blue bricks.  

![code2](movement_block.jpg)

### The Greeting block
Your Spike Dog should be able to recognize you and greet you by wagging it's tail and head. You will use the *colour sensor* for this. To start, define a brick and call it *"Begrüßung"*. First, use the *"stoppe andere Stapel"*-brick to prevent instructions from other blocks from interfering with this process. Then the blue brick stops the movement. After a short pause for one second (yellow brick) you should use a *control brick* to repeat the action two times. The action that should be repeated has to placed inside the control brick. Here, you want the Spike dogs eyes to blink and the display to show a smile. The purple brick starts the barking sound. Then you use a *motor brick* to activate the wagging of the tail and head. Try out different degrees and choose what looks best for you! 

**Please be careful: Watch out which motor you choose here!**

As we said, this action will be repeated twice. After a short pause, we use a *motor brick*, to straighten the position of the head again. This is important since the head and eyes are actually our *distance sensor*. If the sensor is not aligned straight, it can't work properly and will be prone to malfunctions. 

![code2](greeting_block.jpg)

### The Stranger block
You also want to define what happens if your Spike dog recognizes a stranger. Again you should use a *define brick* and stop the other blocks (yellow brick). Just as before, you start by stopping the movement and wait (blue and yellow bricks). Again, you use a *repeat brick*, but now you use a *sound brick* to start the barking sound and a *light brick* to write "STOP!" at the display. 
...
But we wanted more functionality. That's why we advise to use an *falls-dann control brick*. Under *sensors* you will find the brick that we will us as our condition: "if our color sensor F will detects a specific color". Then define, what should happen if that condition appears: It will go to the function blocks you defined earlier. 
...
### The Heel block
Right now, Spike Dog is theoretically able to walk forward, dodge things and react to colors. But what if you want to take it for a walk? 
...
We learned a lot about the sensors (distance, color and force sensors). We encountered problems that we learned to deal with. For example, the short distance of the color perception of the color sensor and that the obstacles for the distance sensor cannot be made of just any material, as it uses ultrasonic technology (therefore don't use sound-absorbing materials). 

For future projects and optimized social human-machine interaction, we would love to use an AI camera for better recognition of the environment (especially people) and a microphone for voice commands. However, these aren't included in the [LEGO® Education SPIKE™ Prime-Set](https://education.lego.com/de-de/products/lego-education-spike-prime-set/45678/). With an AI camera, you could also improve the heel function. In our case it is only able to follow in a straight line right now.

Furthermore, the movement could be made more dog-like by replacing the wheels and using joints to represent the legs. 

But we're happy that we've built a robotic dog with the main focus on social interaction with a few functioning features. Have fun if you want to build the puppy-like robot :smile:
